<template>
  <div class="relative" ref="rootRef">
    <button
      type="button"
      :disabled="disabled"
      class="inline-flex h-12 w-full items-center justify-between gap-3 rounded-2xl border px-4 text-sm font-medium transition-all duration-smooth"
      :class="disabled
        ? 'cursor-not-allowed border-border bg-surfaceSoft text-text-light shadow-none'
        : open
          ? 'border-brand-primary bg-white text-brand-primary shadow-sm'
          : 'border-border bg-white text-text-main shadow-sm hover:border-brand-primary hover:text-brand-primary'"
      @click="togglePanel"
    >
      <div class="flex min-w-0 items-center gap-2">
        <svg
          class="h-4 w-4 shrink-0"
          :class="disabled ? 'text-text-light' : modelValue ? 'text-brand-primary' : 'text-text-light'"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
        ...
        <span class="truncate" :class="disabled ? 'text-text-light' : modelValue ? 'text-text-main' : 'text-text-light'">
          {{ displayText }}
        </span>
      </div>
      ...
      <svg
        ...
        :class="[
          disabled ? 'text-text-light' : 'text-text-muted',
          open ? 'rotate-180' : ''
        ]"
        fill="none"
        stroke="currentColor"
    ...
    <div
      v-if="open"
      class="absolute left-0 top-[calc(100%+12px)] z-50 w-full overflow-hidden rounded-[28px] border border-border bg-white shadow-cardHover sm:w-[460px]"
    >
      <div class="border-b border-border bg-surfaceSoft/60 px-5 py-4">
        <p class="text-base font-semibold text-text-main">选择城市</p>
      </div>
      ...
      <div v-else class="px-4 py-4">
        ...
        <div class="flex items-center gap-4">
          <div ref="listRef" class="max-h-[400px] flex-1 overflow-y-auto pr-1" @scroll="handleListScroll">
            <div
...
const { cityGroups, cityLoading, ensureCityGroups } = useCity()
...
const resolveLetterClass = (letter) => {
  if (!letterSet.value.has(letter)) {
    return 'cursor-default text-text-light'
  }
...
watch(() => groupedCities.value.length, async (length) => {
